...
CREATE TABLE `places` (
    ...
    `opening_hours` VARCHAR(100) NULL COMMENT '영업시간',
...
CREATE TABLE `travel_routes` (
     ...
     `title` VARCHAR(100) NOT NULL COMMENT '여행 경로 제목',
...
-- Full-Text Index 추가 (이름과 주소로 검색 성능 향상)
-- 한글 검색 최적화를 위해 ngram 파서 사용 권장
CREATE FULLTEXT INDEX idx_places_fulltext ON places(name, address) WITH PARSER ngram;